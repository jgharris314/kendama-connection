...
function TrickGenerators() {
  ...
  const [trickList, setTrickList] = useState({"id": 0, "tricks": [], "name": "Pick or create a tricklist."})
        ...
        <div className="row">
          ...
          <Switch>
            <route path="/trick-generators/open">
              <h6>Presets</h6>
              {/* <ListPresets trickListsArr={trickListsArr} /> */}
              {trickListsArr.map((t) => <li key={t.id} onClick={() => loadTrickList(t.id)}>{t.name}</li>)}
          ...
          <Switch>
            <route path="/trick-generators/open">
              ...
              {trickList.tricks.map((t) => <li>{t}</li>)}
